...
local function get_window_config()
	...
	return {
		header = {
			...
			height = 1,
			style = "minimal",
			border = config.options.window.header.border,
		...
		body = {
			...
			height = body_height,
			style = "minimal",
			border = config.options.window.body.border,
			col = math.floor((width - window_width) / 2),
			row = 8,
		},
	}
...
local function setup_keymaps()
	...
	utils.set_keymap("n", config.options.keymaps.remove_completed, actions.remove_completed_todos)
